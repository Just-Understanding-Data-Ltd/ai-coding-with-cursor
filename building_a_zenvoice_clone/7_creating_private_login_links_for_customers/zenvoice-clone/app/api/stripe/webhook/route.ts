      ...
      case "checkout.session.completed": {
        ...
        if (userId) {
          ...
          if (session.mode === "subscription") {
            ...
            // For one-time payments, we need to get the price ID from the line items
          ...
          if (priceId) {
            try {
              ...
              logger.info(
                `Updated user ${userId} with subscription status and price ID ${priceId}`
              );
            } catch (error) {
              logger.error(
            ...
            logger.error(
              `Failed to retrieve price ID for session ${session.id}`
            );
          }
        } else {
